...
function _get_url_new($url) { 

$full_content_paragraph = '5'; // максимальное количество абзацев текста с каждого сайта:
$full_content_strlen_min = '50'; // мин количество абзацев текста с каждого сайта:
$full_content_strlen_max = '600'; // максимальная длина абзаца текста для сохранения:
$full_content_tags = '<p><ul><ol><li><br><br /><i>'; // оставлять в контенте теги:
...
if (mb_detect_encoding($result) != 'UTF-8') {
	$result = @iconv(mb_detect_encoding($result), "UTF-8", $result);
}
$result = preg_replace('!\<\s*textarea[^>]*\>.*?\</textarea\>!siu', '', $result);
$result = preg_replace('!\<\s*style[^>]*\>.*?\</style\>!siu', '', $result);
$result = preg_replace('!\<\s*script[^>]*\>.*?\</script\>!siu', '', $result);
$result = preg_replace('!\<\s*head[^>]*\>.*?\</head\>!siu', '', $result);
preg_match_all('!\<p\>(.*?)\</p\>!siu', $result, $lines);

$i2 = 1;
$full_content_content = '';

foreach ($lines[0] as $p) {
	//echo $p;
	$p = trim(strip_tags($p, $full_content_tags));
	$p =  preg_replace("/<([a-z][a-z0-9]*)[^>]*?(\/?)>/i",'<$1$2>', $p);
	$p = trim($p);
	
	if (mb_strlen($p, 'utf-8') > $full_content_strlen_min AND mb_strlen($p, 'utf-8') < $full_content_strlen_max) {
		$full_content_content.= $p;
		if ($i2 == $full_content_paragraph) break;
		$i2++;
	}
}

	if ($full_content_content != '') {
		//$full_content_array[md5($full_content_content)] = $full_content_content;
		$full_content_content = @CleaningFull($full_content_content);
	}

	return trim($full_content_content);
	
}

// обработка контента + полное вырезание ссылок:
function CleaningFull($content) {
$content = ' '.$content.' ';
$badchar_fc = array ("\r","\t",'&nbsp;','&laquo;','&raquo;','&quot;','&#8592;','&#8594;','&#39;','&#8211;','&#32;','&#160;','&#8212;','&#8230;','&#039;','&rarr;','&mdash;','&gt;','&lt;');
$content = str_ireplace($badchar_fc, ' ', $content);
$content = str_ireplace('http://', ' http://', $content);
$content = str_ireplace('https://', ' https://', $content);
$content = str_ireplace('ftp://', ' ftp://', $content);
$content = str_ireplace('...', '.', $content);
$content = str_ireplace(' .', '.', $content);
$content = str_ireplace("\n", ' ', $content);
// поиск и замена недоделанных доменов:
preg_match_all('/(^|[\n ])([\w]*?)((www|ftp)\.[^ \,\"\t\n\r<]*)/siu', $content, $matches0);
foreach ($matches0[0] as $domen) {
$content = str_ireplace(' '.trim($domen), ' http://'.trim($domen), $content);
}
// поиск всех доменов:
preg_match_all('/(^|[\n ])([\w]*?)((ht|f)tp(s)?:\/\/[\w]+[^ \,\"\n\r\t<]*)/siu', $content, $matches1);
$matches = $matches1[0];
// удаление дублей:
$matches = array_unique($matches);
foreach ($matches as $domen) {
$domen = trim($domen);
$content = str_ireplace($domen, '', $content);
} 
